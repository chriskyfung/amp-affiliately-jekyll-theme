...
registerCardImage(async (filePath, metadata) => {
    const image = metadata.fmPreviewImage ?
        metadata.fmPreviewImage : metadata.image ?
            metadata.fmWebviewUrl + metadata.image : metadata.image.path ?
                metadata.fmWebviewUrl + metadata.image.path : `${metadata.fmWebviewUrl}/assets/images/write-2160925.svg`;
