...
Therefore, I created the `picture.html` include as the shortcode for you to include images in AMP valid format properly and quickly.

### Using `picture.html` Include
...
To embed a single video, use the following tag to insert the `youtube.html` include.

```liquid
{% raw %}
{% include youtube.html id="VIDEO_ID" title="VIDEO_TITLE" description="A short description of the video." upload_date="YYYY-MM-DDThh:mm:ssZ" %}
{% endraw %}
```
...
- **upload_date** - _optional_
- **thumb_quality** - _optional_ (e.g., `mq`, `hq`, `sd`, `maxres`)
- **indent** - _optional_

Example Video:
{% include youtube.html id="8jumRGDgxiY" title="GSP319 Build a Website on Google Cloud: Challenge Lab | 🐱‍🏍 GCP learning tour" description="Learn to deploy a microservices application on Google Kubernetes Engine with this step-by-step GSP319 lab guide." upload_date="2020-06-10T09:45:00Z" %}

To embed a playlist, set both the _playlist ID_ and the _ID of the first video within the playlist_.

```liquid
{% raw %}
{% include youtube.html id="VIDEO_ID" playlist="PLAYLIST_ID" title="PLAYLIST_TITLE" description="A short description of the playlist." upload_date="YYYY-MM-DDThh:mm:ssZ" %}
{% endraw %}
```
...
{% include youtube.html id="C49jYtIMAsg" playlist="PLmQ8R1QWYBV72R3JoEWzIH1onqIiai6d0" title="Demos of Qwiklabs Challenge Labs - YouTube" description="A playlist of demo videos for Qwiklabs Challenge Labs." upload_date="2021-06-25T08:04:50Z" %}
